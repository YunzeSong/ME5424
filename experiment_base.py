...
def run_simulation(tasks_data, agents_data, algorithm):
    ...
    if isinstance(tasks_data[0], Task):
        ...
        tasks = [
            Task(tid, (x, y), reward, release, duration, req_type)
    ...
    while events:
        ...
        if free_agents and waiting_tasks:
            ...
            assignments = algorithm.assign_tasks(
                waiting_tasks, free_agents, current_time=current_time
            )
            # 获取算法执行过程的指标并累加
            ...
            for agent, task in assignments:
                ...
                event_id = next(_event_counter)
                heapq.heappush(
                    events, (finish_time, 2, event_id, "task_finish", (agent, task))
                )
...
if __name__ == "__main__":
    ...
    for name, (avg, var) in summary.items():
        print(
            f"{name}\t{avg['total_cost']:.2f}\t{var['total_cost']:.2f}\t{avg['tasks_completed']:.1f}/{num_tasks}\t"
