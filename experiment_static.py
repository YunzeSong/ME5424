...
def run_static_simulation(num_tasks, num_agents, algorithm):
    ...
    comp_time = (
        algorithm.last_computation_time
        if hasattr(algorithm, "last_computation_time")
        else 0.0
    )
    messages = (
        algorithm.last_communication_cost
...
def main():
    ...
    for sc in scenarios:
        print(
            f"Running scenario: {sc['name']} (Tasks: {sc['tasks']}, Agents: {sc['agents']}), Runs: {sc['runs']}"
        )
    ...
    for sc in scenarios:
        ...
        total_costs = [
            avg_results[scenario_name][alg]["total_cost"] for alg in alg_names
        ]
        complete_rates = [
            avg_results[scenario_name][alg]["complete_rate"] for alg in alg_names
        ]
    ...
    for sc in scenarios:
        ...
        for alg in alg_names:
            ...
            print(
                f"{alg}\t{res['total_cost']:.2f}\t{res['complete_rate']:.2f}\t{res['comp_time']:.4f}\t{res['messages']:.1f}\t{res['fairness']:.2f}"
            )
        print("\n")
